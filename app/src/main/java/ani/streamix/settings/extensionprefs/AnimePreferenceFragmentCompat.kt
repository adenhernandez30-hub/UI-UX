package ani.streamix.settings.extensionprefs
...
class AnimeSourcePreferencesFragment : PreferenceFragmentCompat() {
    ...
    private fun populateAnimePreferenceScreen(): PreferenceScreen {
        ...
        val downloadResPref = androidx.preference.Preference(requireContext()).apply {
            ...
            val current = ani.streamix.settings.saving.PrefManager.getPreferredDownloadResolutions(source.name)
            summary = current.joinToString(", ")
            isIconSpaceReserved = false
            setOnPreferenceClickListener {
                ani.streamix.settings.ResolutionPriorityDialog.show(requireContext(), source.name) { updated ->
