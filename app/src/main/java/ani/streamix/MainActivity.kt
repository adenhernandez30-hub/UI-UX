...
import ani.streamix.media.CalendarFragment
import ani.streamix.media.user.LibraryFragment
import ani.streamix.profile.ProfileHostFragment
import ani.streamix.profile.activity.ActivityFragment
import ani.streamix.profile.activity.ActivityFragment.Companion.ActivityType
...
import ani.streamix.others.CustomBottomDialog
...
import ani.streamix.settings.AddRepositoryBottomSheet
import ani.streamix.settings.ExtensionsActivity
import ani.streamix.settings.saving.PrefManager
import ani.streamix.settings.saving.PrefManager.asLiveBool
import ani.streamix.settings.saving.PrefName
import ani.streamix.settings.saving.SharedPreferenceBooleanLiveData
...
import ani.streamix.util.AudioHelper
...
class MainActivity : AppCompatActivity() {
        ...
        binding.root.doOnAttach {
            ...
            selectedOption = if (intent.getBooleanExtra("goToHome", false)) {
                ...
                when (fragment) {
                    ...
                    HomeFragment::class.java.name -> 2
                    MangaFragment::class.java.name -> 0
                    else -> 0
                }
            } else {
    ...
    private fun handleViewIntent(intent: Intent) {
        ...
        try {
            ...
            if (schemeLower == "magnet" || uri.toString().endsWith(".torrent", ignoreCase = true) || intent.type == "application/x-bittorrent") {
                ani.streamix.torrent.DirectTorrentBottomSheet.newInstance(uri.toString())
            ...
            if (uri.scheme.equals("dantotsu", ignoreCase = true)) {
                startActivity(Intent(this, ani.streamix.connections.anilist.UrlMedia::class.java).setData(uri))
                return
            }
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if (intent.getBooleanExtra("goToHome", false)) {
            selectedOption = 2
            binding.includedNavbar.navbar.selectTabAt(2)
