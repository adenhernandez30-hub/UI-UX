...
import ani.streamix.bottomBarOrNull
import ani.streamix.download.anime.OfflineAnimeAdapter
import ani.streamix.download.anime.OfflineAnimeModel
import ani.streamix.download.anime.OfflineAnimeSearchListener
...
class LocalFragment : Fragment(), OfflineAnimeSearchListener {
        ...
        CoroutineScope(Dispatchers.IO + scanJob).launch {
            try {
                ...
                val localItems = animesPage?.animes?.map { sAnime ->
                    ...
                    val cachedCover = ani.streamix.settings.saving.PrefManager
                        .getCustomVal<String>("local_cover_${title}", "")
                        .takeIf { s -> s.isNotEmpty() && s != "null" }
                    val cachedBanner = ani.streamix.settings.saving.PrefManager
                ...
                val appDownloadsManager = ani.streamix.download.DownloadsManager(requireContext())
    ...
    private fun createMediaFromLocalAnime(item: OfflineAnimeModel): Media {
        ...
        return Media(
            ...
            anime = ani.streamix.media.anime.Anime(
                totalEpisodes = item.totalEpisode.toIntOrNull()
            ),
        ).also {
            it.format = "LOCAL"
            it.folderName = item.folderName
            it.cover = ani.streamix.settings.saving.PrefManager.getCustomVal<String>("local_cover_${item.title}", "")
                .takeIf { s -> s.isNotEmpty() && s != "null" } ?: item.image?.toString()
            it.banner = ani.streamix.settings.saving.PrefManager.getCustomVal<String>("local_banner_${item.title}", "")
    ...
    private fun createMediaFromLocalManga(item: OfflineAnimeModel): Media {
        ...
        return Media(
            ...
            status = item.status ?: "UNKNOWN",
            genres = item.genres?.split(",")?.map { it.trim() } as? ArrayList<String> ?: arrayListOf(),
            manga = ani.streamix.media.manga.Manga(
    ...
    private fun createMediaFromLocalNovel(item: OfflineAnimeModel): Media {
        val localSourceIndex = ani.streamix.parsers.NovelSources.list.indexOfFirst { it.name == "Local" }
            .takeIf { it >= 0 } ?: 0
