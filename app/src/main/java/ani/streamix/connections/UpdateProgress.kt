package ani.streamix.connections
...
fun updateProgress(media: Media, number: String) {
    ...
    if (autoDelete) {
        ...
        val targetNum = ani.streamix.media.MediaNameAdapter.findChapterNumber(number)
        val downloadedType = downloadedTypes.find {
            it.titleName == media.mainName().findValidName() &&
            (it.chapterName == number.findValidName() ||
             (targetNum != null && ani.streamix.media.MediaNameAdapter.findChapterNumber(it.chapterName) == targetNum))
        }
    ...
    val progressInt = ani.streamix.media.MediaNameAdapter.findEpisodeNumber(number)?.toInt()
