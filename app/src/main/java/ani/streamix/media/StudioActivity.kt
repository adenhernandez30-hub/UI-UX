...
import ani.streamix.EmptyAdapter
import ani.streamix.R
import ani.streamix.Refresh
import ani.streamix.databinding.ActivityStudioBinding
...
class StudioActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var isFav = false
        binding.studioFav.visibility = if (ani.streamix.connections.anilist.Anilist.token != null) View.VISIBLE else View.GONE
        scope.launch(Dispatchers.IO) {
            val studioId = studio?.id?.toIntOrNull() ?: return@launch
            isFav = ani.streamix.connections.anilist.Anilist.query.isUserFav(
                ani.streamix.connections.anilist.AnilistMutations.FavType.STUDIO,
                studioId
            )
        ...
        binding.studioFav.setOnClickListener {
            ...
            scope.launch(Dispatchers.IO) {
                val success = ani.streamix.connections.anilist.Anilist.mutation.toggleFav(
                    ani.streamix.connections.anilist.AnilistMutations.FavType.STUDIO,
                    studioId
                )
                withContext(Dispatchers.Main) {
                    if (success) {
                        ...
                        ani.streamix.snackString(if (isFav) "Added to favorites" else "Removed from favorites")
                    }
                }
            }
        }
