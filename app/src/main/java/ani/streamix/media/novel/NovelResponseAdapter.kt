...
import ani.streamix.databinding.ItemChapterListBinding
import ani.streamix.databinding.ItemEpisodeCompactBinding
import ani.streamix.databinding.ItemNovelResponseBinding
...
class NovelResponseAdapter(
            ...
            binding.itemDownload.setOnClickListener {
                if (activeDownloads.contains(novel.link)) {
                    ...
                    downloadTriggerCallback.downloadTrigger(
                        ani.streamix.media.novel.NovelDownloadPackage(
                            novel.link,
                            novel.coverUrl.url,
                            novel.name,
                            novel.link
                ...
                if (0 <= bindingAdapterPosition && bindingAdapterPosition < list.size) {
                    ...
                    if (activeDownloads.contains(currentNovel.link) || downloadedCheckCallback.downloadedCheck(currentNovel) || downloadedChapters.contains(currentNovel.link)) {
                        it.context.customAlertDialog().apply {
                            ...
                            val input = android.widget.NumberPicker(ani.streamix.currContext())
                            input.minValue = 1
    ...
    inner class CompactViewHolder(val binding: ItemEpisodeCompactBinding) :
        ...
        fun bind(novel: ShowResponse, position: Int) {
            ...
            val parsedNumber = ani.streamix.media.MediaNameAdapter.findChapterNumber(chapterNumStr)
        ...
        for (i in 0 until n) {
            if (position + i < list.size) {
                ...
                if (activeDownloads.contains(novel.link)) {
                    ...
                    downloadTriggerCallback.downloadTrigger(
                        ani.streamix.media.novel.NovelDownloadPackage(novel.link, novel.coverUrl.url, novel.name, novel.link)
                    )
