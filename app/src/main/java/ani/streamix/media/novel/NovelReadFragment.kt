package ani.streamix.media.novel
...
import ani.streamix.download.novel.NovelDownloaderService
import ani.streamix.download.novel.NovelServiceDataSingleton
...
import ani.streamix.media.novel.novelreader.NovelReaderActivity
...
import ani.streamix.util.StoragePermissions
...
class NovelReadFragment : Fragment(),
    ...
    private var currentChapterLinks: List<ani.streamix.FileUrl> = emptyList()
    ...
    override fun downloadTrigger(novelDownloadPackage: NovelDownloadPackage) {
        ...
        activity?.let {
            fun continueDownload() {
                if (PrefManager.getVal<Boolean>(PrefName.DownloadWifiOnly) && !ani.streamix.isWifiConnected(it)) {
                    snackString(getString(R.string.download_wifi_only_warning))
                    return
                }
                val parser = model.novelSources[source] as? ani.streamix.parsers.novel.LnReaderNovelParser
    ...
    fun onNovelClick(novel: ShowResponse) {
        ...
        ani.streamix.settings.saving.PrefManager.setCustomVal("${media.id}_last_read_volume", novel.name)
            ...
            if (it != null) {
                ...
                if (!loaded) {
                    ...
                    val isLnReader = source in 0 until model.novelSources.names.size
                            && model.novelSources[source] is ani.streamix.parsers.novel.LnReaderNovelParser
                    if (isLnReader) {
            ...
            if (loadedChapters != null && loadedChapters.containsKey(source)) {
                ...
                if (!chapters.isNullOrEmpty()) {
                    ...
                    currentChapterLinks = chapters.map { ch ->
                        ...
                        ani.streamix.FileUrl(ch.link, headers)
                    }
                    updateChaptersTabs()
                    novelResponseAdapter.clear()
                    var chunk = chapters
    ...
    fun multiDownload(n: Int) {
        lifecycleScope.launch {
            ...
            val progressChapterIndex = (chapters.indexOfFirst {
                ani.streamix.media.MediaNameAdapter.findChapterNumber(it.name)?.toInt() == selected
            } + 1).coerceAtLeast(0)
            ...
            for (chapter in chaptersToDownload) {
                try {
                    ...
                    ani.streamix.snackString("Failed to download chapter: ${chapter.name}")
                }
            }
            ani.streamix.snackString("All downloads completed!")
        }
    }
