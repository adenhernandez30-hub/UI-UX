...
import ani.streamix.databinding.FragmentMediaSourceBinding
import ani.streamix.download.DownloadedType
import ani.streamix.download.DownloadsManager
import ani.streamix.download.DownloadsManager.Companion.compareName
import ani.streamix.download.DownloadsManager.Companion.getSubDirectory
import ani.streamix.download.anime.AnimeDownloader
import ani.streamix.download.anime.AnimeDownloaderService
import ani.streamix.download.findValidName
import ani.streamix.dp
import ani.streamix.isOnline
import ani.streamix.media.Media
import ani.streamix.media.MediaDetailsActivity
...
import ani.streamix.notifications.subscription.SubscriptionHelper.Companion.saveSubscription
import ani.streamix.others.LanguageMapper
import ani.streamix.parsers.AnimeParser
import ani.streamix.parsers.AnimeSources
import ani.streamix.parsers.HAnimeSources
import ani.streamix.setBaseline
import ani.streamix.setNavigationTheme
import ani.streamix.toPx
import ani.streamix.settings.extensionprefs.AnimeSourcePreferencesFragment
...
import ani.streamix.util.StoragePermissions.Companion.accessAlertDialog
import ani.streamix.util.StoragePermissions.Companion.hasDirAccess
...
class AnimeWatchFragment : Fragment(), AnimeWatchAdapter.ScanlatorSelectionListener {
    ...
    fun openDirectTorrent() {
        ...
        val sheet = ani.streamix.torrent.DirectTorrentBottomSheet.newInstanceLinked(media, torrentUrl)
        sheet.onTorrentSelected = { chosenUrl ->
            ...
            if (torrentIndex != -1) {
                ...
                val showResponse = ani.streamix.parsers.ShowResponse(
                    name = title,
                    link = chosenUrl,
                    coverUrl = ani.streamix.FileUrl(media.cover ?: media.banner ?: ""),
                    sAnime = sAnime
                )
    ...
    fun onAnimeEpisodesDownload(episodesToDownload: ArrayList<String>) {
        if (PrefManager.getVal<Boolean>(PrefName.DownloadWifiOnly) && !ani.streamix.isWifiConnected(requireContext())) {
    ...
    companion object {
        const val ACTION_DOWNLOAD_STARTED = "ani.streamix.ACTION_DOWNLOAD_STARTED"
        const val ACTION_DOWNLOAD_FINISHED = "ani.streamix.ACTION_DOWNLOAD_FINISHED"
        const val ACTION_DOWNLOAD_FAILED = "ani.streamix.ACTION_DOWNLOAD_FAILED"
        const val ACTION_DOWNLOAD_PROGRESS = "ani.streamix.ACTION_DOWNLOAD_PROGRESS"
