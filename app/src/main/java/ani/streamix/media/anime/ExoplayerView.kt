package ani.streamix.media.anime
...
import ani.streamix.download.DownloadsManager
import ani.streamix.download.anime.AnimeDownloader
import ani.streamix.download.video.Helper
...
import ani.streamix.NoPaddingArrayAdapter
...
import ani.streamix.connections.subtitles.OpenSubRestItem
import ani.streamix.connections.subtitles.OpenSubtitlesRestApi
import ani.streamix.connections.subtitles.StremioSub
import ani.streamix.connections.subtitles.StremioSubtitles
import ani.streamix.connections.subtitles.SubSourceSub
import ani.streamix.connections.subtitles.SubSourceSubtitles
import ani.streamix.connections.subtitles.WyzieSub
import ani.streamix.connections.subtitles.WyzieSubtitles
import ani.streamix.databinding.ActivityExoplayerBinding
import ani.streamix.dp
import ani.streamix.hideSystemBars
import ani.streamix.hideSystemBarsExtendView
import ani.streamix.isOnline
import ani.streamix.media.EpisodeMapper
import ani.streamix.media.Media
import ani.streamix.media.MediaDetailsViewModel
import ani.streamix.media.MediaNameAdapter
import ani.streamix.media.anime.player.CastScreenView
import ani.streamix.media.anime.player.DantotsuPlayerManager
import ani.streamix.media.anime.player.PlayerAniSkipManager
import ani.streamix.media.anime.player.PlayerCastManager
import ani.streamix.media.anime.player.PlayerDiscordManager
import ani.streamix.media.anime.player.PlayerGestureManager
import ani.streamix.media.anime.player.PlayerProgressManager
import ani.streamix.media.anime.player.PlayerScreenshotManager
import ani.streamix.media.anime.player.PlayerSubtitleManager
import ani.streamix.shareImage
import ani.streamix.others.IdMappers
import ani.streamix.others.LanguageMapper
import ani.streamix.others.Xubtitle
import ani.streamix.others.getSerialized
import ani.streamix.parsers.AnimeSources
import ani.streamix.parsers.HAnimeSources
import ani.streamix.parsers.Subtitle
import ani.streamix.parsers.Video
import ani.streamix.parsers.VideoExtractor
import ani.streamix.parsers.VideoType
import ani.streamix.settings.PlayerSettingsActivity
...
class ExoplayerView : AppCompatActivity(), Player.Listener {
    ...
    companion object {
        ...
        const val ACTION_PIP_PREV_EP = "ani.streamix.PIP_PREV_EP"
        const val ACTION_PIP_REWIND = "ani.streamix.PIP_REWIND"
        const val ACTION_PIP_PLAY_PAUSE = "ani.streamix.PIP_PLAY_PAUSE"
        const val ACTION_PIP_SKIP = "ani.streamix.PIP_SKIP"
        const val ACTION_PIP_NEXT_EP = "ani.streamix.PIP_NEXT_EP"
    }
    ...
    private fun initPlayer() {
        ...
        val downloadedMediaItem = if (ext.server.offline) {
            ...
            val directory = ani.streamix.download.DownloadsManager.getSubDirectory(this, ani.streamix.media.MediaType.ANIME, false, titleName, episodeName)
            ...
            val encrypted = directory?.let {
                ani.streamix.download.video.DrmDownloader.offlineMediaItem(this, it)
            }
            if (encrypted != null) {
        ...
        if (drmInfo?.offline != null) {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                runCatching {
                    val dir = ani.streamix.download.DownloadsManager.findSubDirectory(
                        this@ExoplayerView, ani.streamix.media.MediaType.ANIME, title, epNumber
                    ) ?: return@runCatching
                    ani.streamix.download.video.DrmDownloader.refreshIfStale(
    ...
    private fun findBestVideoForDownload(
        videos: List<ani.streamix.parsers.Video>,
        preferredResolutions: List<String>
    ): ani.streamix.parsers.Video? {
    ...
    private fun checkSmartDownloadAnime(currentExt: ani.streamix.parsers.VideoExtractor) {
    ...
    override fun onDestroy() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                val torrentManager = uy.kohesive.injekt.Injekt.get<ani.streamix.torrent.TorrentServerManager>()
            ...
            try {
                ani.streamix.addons.torrent.TorrentServerService.stop()
            } catch (_: Exception) {}
        }
        aniSkipManager.stopTracking()
        progressManager.stopTracking()
